...
public class CLog {
	...
	synchronized public static void writeLog(String logmsg) {
		...
		SimpleDateFormat sf = new SimpleDateFormat("yyyyMMdd HH:mm:ss.SSS",Locale.US);
		try {
	...
	synchronized public static void writeLogVS(String logmsg) 
	{
		App.Mkdir(path);
		Date currTime = new Date();
		SimpleDateFormat sf = new SimpleDateFormat("yyyyMMdd HH:mm:ss.SSS",Locale.US);
		try {
			out = new FileOutputStream(path+File.separator+App.getCurDateStr(11)+"_VS"+".log",true);
			p = new PrintWriter(out);
			p.println(sf.format(currTime) + " " + logmsg);
			p.close();
			out.close();
