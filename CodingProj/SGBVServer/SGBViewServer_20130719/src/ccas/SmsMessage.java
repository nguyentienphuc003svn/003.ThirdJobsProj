...
*/
		...
		try {
			if	(srv!=null){
				if (!srv.getConnected()){
					...
					srv.setSimPin("0000");
					srv.connect();
					System.out.println(ccas.App.getCurDateStr(19)+"	Signal Level  : "+ srv.getDeviceInfo().getSignalLevel() + "%");
					srv.setSmscNumber("");
			...
			CLog.writeLog("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa ");
	...
	public boolean sendMessages(String mobilePhoneNum, String message)	throws Exception {
		...
		//System.out.print("So dien thoai" + mobilePhoneNum);	
		...
		try {
			...
			e.getMessage();
			//System.out.println(ccas.App.getCurDateStr(19)+"	length="+temp.length()+",msg["+temp+"],mobile="+mobilePhoneNum);
			System.out.print("aaaaaaaaaaaaaaa" + e.getMessage());
			retboolean=false;
		}	finally	{
			if	(retboolean){
				CLog.writeLog("mobile:"+mobilePhoneNum+",length=["+message.length()+"],Processing time:"+(System.currentTimeMillis() - stime)+" ms");
				System.out.println(ccas.App.getCurDateStr(19)+"	mobile:"+mobilePhoneNum+",length=["+message.length()+"],Processing time:"+(System.currentTimeMillis() - stime)+" ms");
			}
		}
		return false;
	}
	...
	public static void main(String[] args) {
		try {
			...
			sendMessage.sendMessages("0988566151","test1 for SMS,send time is "+App.getCurDateStr(21));
